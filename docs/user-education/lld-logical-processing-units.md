...
Logical Processing Units (LPUs) are the **atomic business operations** that make up a [[lld-flows]]'s processing logic. They break down complex business functionality into smaller, reusable chunks.
...
| LLM Assisted | Code generated from the business logic description with [[nurture-overview]] help           |
...
| Sub-Type        | The functional classification: `Validation`, `Enrichment`, `Host Call`, `Processing`, `Persistence`, `SetDefaults`, `Transformation`, `Complete`. Used to optimise chaining of LPUs in Flows. Input / Output operations need to be at the edges of the [[lld-flows]] |
| Input           | The [[lld-data-models]] consumed by this LPU (e.g., `PartyDTO`, `PartyProfileAttributeDTO`)                                                                                                                                                                          |
| Output          | The [[lld-data-models]] produced by this LPU (e.g., `PartyResponseDTO`). Leave blank for side-effect-only operations like persistence                                                                                                                                |
| Referenced Data | [[lld-data-models]] used by this LPU during processing that come from other sources — not provided as input to the service (e.g., looking up existing records from the database)                                                                                     |
...
- Reference **data models by name** — use the exact model names from [[lld-data-models]] in the Input and Output columns for traceability
...
- [[lld-services]] — Previous section: services that orchestrate LPUs
- [[lld-test-data]] — Next section: test payloads
