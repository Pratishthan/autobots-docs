...
The Data Models section defines all the **data structures** involved in your feature — whether data at rest, data in motion, or data at the API boundary. This is typically the first technical section you fill in after [Background](lld-background.md).
...
- **Indexes** — Which columns are indexed and why. These should align with the queries defined in the [Data Access](../reference/lld-template.md) section
...
| Data Type | The data type of this field. Can be: (1) an OpenAPI primitive (`String`, `Integer`, `Boolean`, `Number`), (2) a **platform type** from the FBP type registry (e.g., `BusinessDateDTO`, `AmountDTO` — see [Fbp Data Types](../reference/fbp-data-types.md)), or (3) a **reference to another model** defined in this LLD (`ContactDetails`, `List<PartyProfileAttribute>`) |
...
| Business Name      | The human-readable, business-friendly label for this field (e.g., `partyNumber` → "Party Number"). Used in generated UI labels, [Lld Test Scenarios](lld-test-scenarios.md) and reports                                                                                |
...
The full template also includes Domain Extensions for Node-RED and Python — see [Lld Template](../reference/lld-template.md).

### Enumerations

Enumerations define fixed value sets referenced by data model attributes. Listed as section 3 in the [template](../reference/lld-template.md) but documented here alongside data models since they are tightly coupled.
...
- [Lld Background](lld-background.md) — Previous section: business context
- [Lld Services](lld-services.md) — Next section: defining services
