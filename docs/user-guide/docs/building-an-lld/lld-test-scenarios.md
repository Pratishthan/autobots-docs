...
The Pre-conditions (Given) and Steps (When) columns frequently reference **test data sets** from [Lld Test Data](lld-test-data.md) with specific overrides. For example:
...
| Service / Unit | Which service this scenario tests — references a service name from [Lld Services](lld-services.md). Scenarios test services, not individual LPUs |
| Priority | Testing priority: `L1` (critical — must pass), `L2` (important — should pass), `L3` (nice to have — edge cases and rare paths) |
| Preconditions | Initial state required before the test — often references test data sets from [Lld Test Data](lld-test-data.md) with specific overrides |
...
| Reference Models | Data models involved in this scenario — for traceability back to [Lld Data Models](lld-data-models.md) |
...
- Reference **test data sets by name** from [Lld Test Data](lld-test-data.md) rather than inlining sample data in the scenario
...
- [Lld Test Data](lld-test-data.md) — Previous section: reusable test payloads
